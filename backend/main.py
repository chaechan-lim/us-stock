...
async def lifespan(app: FastAPI):
    ...
    evaluation_loop._disabled_strategies = frozenset(
        s for s in _all_strategies if s not in _us_enabled
    )
    evaluation_loop._min_confidence = 0.30
    evaluation_loop._min_active_ratio = 0.0
    logger.info(
        "US eval loop: enabled=%s, disabled=%d strategies, min_confidence=%.2f",
        sorted(_us_enabled), len(evaluation_loop._disabled_strategies), 0.30,
    )
