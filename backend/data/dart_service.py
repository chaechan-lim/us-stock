...
class DARTInsiderService:
    ...
    def __init__(
        ...
        # Lazy-loaded corp_code map (stock_code -> 8-digit corp_code).
        # Generated by scripts/fetch_dart_corp_codes.py.
        self._corp_map: dict[str, str] = {}
        default_map = Path("data/dart_corp_map.json")
        path = Path(corp_map_path) if corp_map_path else default_map
        if path.exists():
            try:
                self._corp_map = json.loads(path.read_text())
                logger.info("DART corp map loaded: %d entries", len(self._corp_map))
            except (OSError, json.JSONDecodeError) as e:
                logger.warning("DART corp map load failed: %s", e)
        if not self._enabled:
            logger.info("DART service disabled (no API key or disabled flag)")
        elif not self._corp_map:
            logger.warning(
                "DART enabled but corp map empty — run scripts/fetch_dart_corp_codes.py"
            )
    ...
    def _stock_to_corp(self, stock_code: str) -> str:
        ...
        Reads from data/dart_corp_map.json loaded at startup. Map is
        generated by scripts/fetch_dart_corp_codes.py (one-time, refresh
        quarterly).
        """
